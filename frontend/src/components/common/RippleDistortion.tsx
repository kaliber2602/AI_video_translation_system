...
const MAX_WAVES = 100;
const QUALITY_SCALE: Record<string, number> = { low: 0.4, medium: 0.7, high: 1.0 };
...
void main() {
  ...
  vec4 texColor = texture2D(uTexture, base + push);
  vec3 color;
  ...
  if (uGlint > 0.001) {
    ...
    float glintVal = clamp((raw - flatSpec) / max(1.0 - flatSpec, 0.0001), 0.0, 1.0) * uGlint;
    color += uHighlight * glintVal;
    waveAlpha = clamp(waveAlpha + glintVal * 0.5, 0.0, 1.0);
  }

  gl_FragColor = vec4(color, waveAlpha * uAlpha);
}
`;
...
function createDefaultGradientCanvas(): HTMLCanvasElement {
...
export default function RippleDistortion({
  ...
  useEffect(() => {
    ...
    if (src) {
      ...
      const gradCanvas = createDefaultGradientCanvas();
      imageTexture.image = gradCanvas;
      compositeUniforms.uTextureSize.value = [gradCanvas.width, gradCanvas.height];
    }
  ...
  useEffect(() => {
    ...
    u.composite.uAlpha.value = alpha;
  }, [rings, strength, swirl, dispersion, glint, tintAmount, grayscale, highlightColor, tint, alpha]);
